...
export default function AuthPage({ mode = "login", onNavigate, onLogin }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Store token and face_enrolled flag for FaceVerify to use
      localStorage.setItem("shield_token",        res.data.access_token);
      localStorage.setItem("shield_face_enrolled", res.data.face_enrolled ? "true" : "false");
  ...
  return (
    <div style={{ minHeight: "100vh", background: t.bg, color: t.text, fontFamily: "'Courier New', monospace", display: "flex", flexDirection: "column" }}>
      <style>{`@keyframes fadeUp { from{opacity:0;transform:translateY(20px)} to{opacity:1;transform:translateY(0)} } @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:0.5} } @keyframes float { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-10px)} }`}</style>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "48px 24px" }}>
        <div style={{ width: "100%", maxWidth: 440, animation: "fadeUp 0.6s ease both" }}>
          ...
          <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 14, padding: "32px 36px", boxShadow: `0 0 40px ${t.green}08` }}>
            {!isLogin && <InputField label="FULL NAME"        value={name}     onChange={setName}     placeholder="Priya Sharma"  t={t} />}
            ...
            {!isLogin && <InputField label="CONFIRM PASSWORD" type="password" value={confirm} onChange={setConfirm} placeholder="••••••••" t={t} />}
            ...
            <div style={{ textAlign: "center", fontSize: 12, color: t.textDim }}>
              ...
              <span onClick={() => onNavigate(isLogin ? "register" : "login")}
                style={{ color: t.green, cursor: "pointer", fontWeight: 700 }}>
          ...
          <div style={{ marginTop: 12, textAlign: "center" }}>
            <span onClick={() => onNavigate("landing")} style={{ fontSize: 11, color: t.textDim, cursor: "pointer" }}>
              ← Back to home
            </span>
          </div>
        </div>
