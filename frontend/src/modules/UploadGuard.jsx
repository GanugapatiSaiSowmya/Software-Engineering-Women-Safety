...
export default function UploadGuard() {
  ...
  const handleDrop = async (e) => {
    ...
    try {
      ...
      if (data.ai_results) {
        setDeepfake(data.ai_results);
      }

      if (data.protected_url) {
        setProtectedUrl(data.protected_url);
      }

      if (data.final_filename) {
        setFinalFilename(data.final_filename);
      }
      ...
      showToast(
        "Neural scan complete — file sanitized.",
        "success"
      );
      ...
      showToast(
        "Security alert: Backend connection failed.",
        "alert"
      );
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          <h3 className="text-lg font-bold tracking-widest text-white">
            NEURAL PROTECTION INTERFACE
          </h3>

          <p className="text-xs text-slate-500 mono mt-1">
            Deepfake / EXIF sandbox · local processing only
          </p>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <motion.div
          layout
          className="glass-card glass-card-mint flex flex-col overflow-hidden"
        >
            ...
            <label
              ...
              className={`flex-1 min-h-[280px] rounded-xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition-all relative overflow-hidden ${
                dragging
                  ...
                  : "border-slate-800 bg-[#121c2a]/80 hover:border-[#2dd4bf]/40"
              }`}
            >
              <input
                type="file"
              ...
              {previewUrl ? (
                <>
                  <img
                    src={previewUrl}
                    alt="Preview"
                    className="max-w-full max-h-60 rounded-lg object-contain z-10"
                  />
                ...
                <div className="text-center p-8">
                  ...
                  <p className="text-slate-300 font-medium mb-2">
                    Drop target imagery here
                  </p>

                  <p className="text-xs text-slate-500 mono">
                    Initialize deepfake &amp; EXIF neural scan
                  </p>
                </div>
              )}
            </label>
                ...
                >
                  <motion.div className="flex items-center gap-3 text-[#2dd4bf] text-sm tracking-wide">
                    <motion.div
                      ...
                      className="w-4 h-4 rounded-full border-2 border-dashed border-[#2dd4bf]"
                    />
                    ...
                    >
                      <div className="flex justify-between mono text-[11px]">
                        ...
                        <span className="text-[#2dd4bf]">
                          {protocolProgress[protocol.id] || 0}%
                        </span>
                      </div>
        ...
        <div className="flex flex-col gap-6">
          {analyzed ? (
            <>
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-400 leading-relaxed">
                      {riskScore > 70
                        ? "High risk elements detected. Immediate purging recommended."
                        : "Threat level acceptable. Defensive measures applied."}
                    </p>
                  </div>
                </div>
              </motion.div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <motion.div
                  ...
                  className="glass-card p-5 bg-[#121c2a]/60"
                >
                  <div className="text-xs font-bold tracking-widest text-amber-500 mb-4 border-b border-white/5 pb-2">
                    PRIVACY TRIGGERS
                  </div>
                ...
                <motion.div
                  ...
                  className="glass-card p-5 bg-[#121c2a]/60"
                >
                  <div className="text-xs font-bold tracking-widest text-[#2dd4bf] mb-4 border-b border-white/5 pb-2">
                    DEFENSE PLAN
                  </div>

                  <ul className="space-y-3 text-xs text-slate-300">
                    <li>
                      <span className="text-[#2dd4bf] mr-2">
                        ✓
                      </span>
                      Metadata Stripped
                    </li>
              ...
              <motion.button
                onClick={() => {
                  if (protectedUrl) {
                    ...
                    showToast(
                      "Protected file is being prepared. Please try again.",
                      "alert"
                    );
                  }
                }}
            ...
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="glass-card flex flex-col items-center justify-center min-h-[380px] gap-4 text-center bg-[#121c2a]/60"
            >
              <Lock
                size={48}
              ...
              <p className="text-xs text-slate-600 max-w-[220px]">
                Upload imagery to initialize security audit and cloaking protocols
              </p>
            </motion.div>
          ) : null}
        </div>
